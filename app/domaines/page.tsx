...
function StaffBadge({ assigned, capacity, name }: { assigned: number; capacity: number; name: string }) {
  ...
  if (assigned >= capacity) return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-green-100 text-green-800">✓ {assigned}/{capacity}</span>;
  if (assigned > 0) return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-800">⚠ {assigned}/{capacity}</span>;
  return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-800">⚠ NON STAFFÉ</span>;
}
